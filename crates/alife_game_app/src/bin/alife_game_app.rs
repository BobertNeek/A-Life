...
use alife_game_app::{
    ...
    run_ca44a_gpu_alpha_stability_smoke, run_cognition_debug_timeline_smoke,
    ...
    validate_app_bundle_manifest, validate_app_shell_config, validate_production_assets,
    validate_production_voxel_save, write_behavior_comparison_lab_report,
    ...
    GpuLongrunSoakOptions, GpuSustainedLearningSoakOptions, ProductionFrontendProfileId,
    ProductionVoxelLaunchConfig, ProductionVoxelLaunchSummary, RuntimePrereqDiagnosticsOptions,
    SampledGpuRuntimeOptions, PRODUCTION_VOXEL_COMMAND,
};
...
fn run() -> Result<String, String> {
    ...
    match args.as_slice() {
        ...
        [command, fixture_root, ticks] if command == "gpu-alpha-stability-smoke" => {
            let ticks = ticks
                .parse::<u32>()
                .map_err(|_| "CA44A stability ticks must be an unsigned integer".to_string())?;
            let launch = AppShellLaunchConfig::from_p34_fixture_root(fixture_root);
            let summary = run_ca44a_gpu_alpha_stability_smoke(&launch, ticks)
                .map_err(|err| err.to_string())?;
            let line = format_ca44a_stability_summary("CA44A gpu alpha stability", &summary);
            summary.validate().map_err(|_| line.clone())?;
            Ok(line)
        }
        ...
        [command, fixture_root] if command == "procedural-world-travel-smoke" => {
            let launch = AppShellLaunchConfig::from_p34_fixture_root(fixture_root);
            let summary =
                run_procedural_world_travel_smoke(&launch).map_err(|err| err.to_string())?;
            Ok(format!(
                "CA44A procedural world travel schema={} version={} seed={} stable={} steps={} unique_chunks={} max_active={} content_seen={} no_render={} rendering_required={} anchor_only={} action_authority={} weight_authority={} claim={} signature={}",
                summary.schema,
                summary.schema_version,
                summary.seed,
                ...
                summary.total_content_candidates_seen,
                summary.generated_without_rendering,
                summary.rendering_required,
                summary.materialized_only_near_creature_anchors,
        ...
        [command] if command == "production-asset-pipeline-smoke" => {
            ...
            Ok(format!(
                "Production asset pipeline schema={} version={} id={} roles={} active_pngs={} blender_script={} launcher_script={} blender_on_path={} blender_discovered={} blender_executable={} status={} user_action_required={} runtime_dependency={} action_authority={} weight_authority={} semantics_authority={} signature={}",
                summary.schema,
                summary.schema_version,
                summary.pipeline_id,
                summary.role_count,
                summary.active_pngs_exist,
                summary.blender_script_present,
                summary.launcher_script_present,
                summary.blender_on_path,
                summary.blender_discovered,
                summary.blender_executable.as_deref().unwrap_or("none"),
                summary.local_render_status,
                summary.user_action_required,
                summary.runtime_game_dependency,
                summary.can_emit_actions,
                summary.can_rewrite_weights,
                summary.can_change_simulation_semantics,
                summary.signature_line()
            ))
        }
        [command] if command == "validate-production-assets" => {
            let summary =
                validate_production_assets(alife_game_app::default_production_asset_manifest_path())
                    .map_err(|err| err.to_string())?;
            Ok(format!(
        ...
        [command, fixture_root] if command == "onboarding-tutorial-smoke" => {
            let launch = AppShellLaunchConfig::from_p34_fixture_root(fixture_root);
            let summary = run_onboarding_tutorial_smoke(&launch).map_err(|err| err.to_string())?;
            Ok(format_onboarding_tutorial_summary(
                "CA40 onboarding tutorial",
                &summary,
            ))
        }
...
fn format_ca44a_stability_summary(
    prefix: &str,
    summary: &alife_game_app::Ca44aTickStabilitySummary,
) -> String {
    format!(
        "{prefix} schema={} version={} requested={} completed={} selected_creature={} first_invalid_tick={:?} first_invalid_status={:?} action={:?}:{:?} target={:?} diagnostic={:?} sealed={} packed={} topology={}/{}/{}/{} parity={} fallback={} terminal_invalid={} recoverable_failures={} debug_wall_ms={:.3} avg_ms_per_tick={:.3} ticks_per_second={:.2} signature={}",
        summary.schema,
        summary.schema_version,
        summary.requested_ticks,
        summary.completed_ticks,
        summary.selected_creature.raw(),
        summary.first_invalid_tick,
        summary.first_invalid_status,
        summary.first_invalid_action_kind,
        summary.first_invalid_action_id.map(|id| id.raw()),
        summary.first_invalid_target.map(|id| id.raw()),
        summary.first_invalid_diagnostic,
        summary.sealed_patches,
        summary.packed_records,
        summary.topology_concepts,
        summary.topology_edges,
        summary.topology_simplexes,
        summary.topology_gaps,
        summary.cpu_shadow_parity_preserved,
        summary.fallback_status,
        summary.terminal_invalid_count,
        summary.recoverable_failure_count,
        summary.debug_wall_ms,
...
fn format_app_bundle_summary(
    ...
    format!(
        "{prefix} schema={} version={} bundle={} scenarios={} entries={} shaders={}/{} legacy_art_descriptors={} alpha_art={} alpha_roles={} production_alpha_art={} true_25d={} true_25d_roles={} production_true_25d={} production_voxel_assets={} production_voxel_generated={} production_voxel_validated={} required={} largest_bytes={} missing_required_rejected={} shader_discovery={} legacy_descriptors_valid={} large_binary_assets={} signature={}",
        summary.schema,
        summary.schema_version,
        summary.bundle_id,
        ...
        summary.true_25d_asset_entries,
        summary.true_25d_required_roles_present,
        summary.production_true_25d_assets,
        summary.production_voxel_asset_entries,
        summary.production_voxel_generated_assets,
        summary.production_voxel_asset_manifest_validated,
...
fn format_onboarding_tutorial_summary(
    prefix: &str,
    summary: &alife_game_app::Ca40OnboardingTutorialSummary,
) -> String {
    format!(
        "{prefix} schema={} version={} checklist={} controls={} food={} hazard={} gpu_fallback={} stable_ids={} display_only={} no_actions={} no_weights={} cpu_shadow_gate={} full_action_authoritative={} signature={}",
        summary.schema,
        summary.schema_version,
        summary.checklist.len(),
        summary.graphical_controls_verified,
        summary.has_food_marker,
        summary.has_hazard_marker,
        summary.gpu_fallback_explanation_visible,
        summary.stable_ids_only,
        summary.display_only,
        summary.no_action_authority,
        summary.no_weight_authority,
        summary.cpu_shadow_gate_visible,
        !summary.no_full_action_authoritative_claim,
        summary.signature_line()
    )
}
...
fn format_production_voxel_summary(prefix: &str, summary: &ProductionVoxelLaunchSummary) -> String {
    ...
    format!(
        ...
        summary.production_asset_manifest_path.display(),
        summary.production_asset_count,
        summary.production_asset_vfx_budget_state,
        summary.production_asset_license_metadata_validated,
...
fn configure_windows_graphical_playground_environment() {
    ...
    match backend_request.as_str() {
        ...
        "vulkan" | "vk" => {
            ...
            eprintln!(
                "Windows graphical backend: ALIFE_GRAPHICS_BACKEND=vulkan selected; injected overlay loader warnings may appear."
            );
        }
        "dx12" | "d3d12" => {
            ...
            if matches!(previous.as_deref(), Some("dx12")) {
                ...
                eprintln!(
                    "Windows graphical backend: WGPU_BACKEND=dx12 explicitly selected; Vulkan remains the default True 2.5D Player View backend on this host."
                );
            }
        }
        "auto" => {
            let previous = env::var("WGPU_BACKEND").ok();
            env::set_var("WGPU_BACKEND", "vulkan");
            if matches!(previous.as_deref(), Some("vulkan")) {
                eprintln!("Windows graphical backend: WGPU_BACKEND=vulkan.");
            } else {
                eprintln!(
                    "Windows graphical backend: auto selected WGPU_BACKEND=vulkan for True 2.5D Player View; set ALIFE_GRAPHICS_BACKEND=dx12 only for DX12 diagnostics."
                );
            }
        }
        other => {
            eprintln!(
                "Windows graphical backend: unknown ALIFE_GRAPHICS_BACKEND={other}; using vulkan for True 2.5D Player View."
            );
            env::set_var("WGPU_BACKEND", "vulkan");
        }
    }
