...
fn release_candidate_gates(
    ...
    vec![
        ...
        ReleaseCandidateGate {
            id: "g23-graphical-playtest-manual".to_string(),
            area: ReleaseCandidateArea::GraphicsManual,
            status: ReleaseCandidateGateStatus::Manual,
            command: "powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run_graphical_playground.ps1".to_string(),
            evidence: "manual graphics support is required for graphical playtest evidence; dry-run is CI-safe only".to_string(),
            manual: true,
            release_blocker: false,
        },
    ]
