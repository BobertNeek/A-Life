//! Versioned playable-sim schema constants split during R13 remediation.

pub const G01_APP_SHELL_SCHEMA: &str = "alife.g01.app_shell.v1";
pub const G01_APP_SHELL_SCHEMA_VERSION: u16 = 1;
pub const G02_VISIBLE_WORLD_SCHEMA: &str = "alife.g02.visible_world.v1";
pub const G02_VISIBLE_WORLD_SCHEMA_VERSION: u16 = 1;
pub const G03_LIVE_BRAIN_LOOP_SCHEMA: &str = "alife.g03.live_brain_loop.v1";
pub const G03_LIVE_BRAIN_LOOP_SCHEMA_VERSION: u16 = 1;
pub const G04_CREATURE_VISUAL_SCHEMA: &str = "alife.g04.creature_visual_state.v1";
pub const G04_CREATURE_VISUAL_SCHEMA_VERSION: u16 = 1;
pub const G05_CAMERA_INSPECTOR_SCHEMA: &str = "alife.g05.camera_selection_inspector.v1";
pub const G05_CAMERA_INSPECTOR_SCHEMA_VERSION: u16 = 1;
pub const G06_PLAYABLE_SURVIVAL_LOOP_SCHEMA: &str = "alife.g06.playable_survival_loop.v1";
pub const G06_PLAYABLE_SURVIVAL_LOOP_SCHEMA_VERSION: u16 = 1;
pub const G07_WORLD_ECOLOGY_SCHEMA: &str = "alife.g07.world_ecology_loop.v1";
pub const G07_WORLD_ECOLOGY_SCHEMA_VERSION: u16 = 1;
pub const G08_POPULATION_SOCIAL_SCHEMA: &str = "alife.g08.population_social_loop.v1";
pub const G08_POPULATION_SOCIAL_SCHEMA_VERSION: u16 = 1;
pub const G08_MAX_POPULATION_CAP: usize = 8;
pub const G09_LIFECYCLE_SCHEMA: &str = "alife.g09.lifecycle_lineage.v1";
pub const G09_LIFECYCLE_SCHEMA_VERSION: u16 = 1;
pub const G09_MAX_LIFECYCLE_POPULATION_CAP: usize = 8;
pub const G10_SCHOOL_MODE_SCHEMA: &str = "alife.g10.school_mode.v1";
pub const G10_SCHOOL_MODE_SCHEMA_VERSION: u16 = 1;
pub const G11_SEMANTIC_PROVIDER_DISPLAY_SCHEMA: &str = "alife.g11.semantic_provider_display.v1";
pub const G11_SEMANTIC_PROVIDER_DISPLAY_SCHEMA_VERSION: u16 = 1;
pub const G12_GPU_PRODUCT_TELEMETRY_SCHEMA: &str = "alife.g12.gpu_product_telemetry.v1";
pub const G12_GPU_PRODUCT_TELEMETRY_SCHEMA_VERSION: u16 = 1;
pub const G13_WORLD_EDITOR_SCHEMA: &str = "alife.g13.world_editor.v1";
pub const G13_WORLD_EDITOR_SCHEMA_VERSION: u16 = 1;
pub const G13_EDITOR_MAX_OBJECTS: usize = 16;
pub const G14_COGNITION_DEBUG_SCHEMA: &str = "alife.g14.cognition_debug_timeline.v1";
pub const G14_COGNITION_DEBUG_SCHEMA_VERSION: u16 = 1;
pub const G14_MAX_TIMELINE_ENTRIES: usize = 16;
pub const G14_MAX_PROPOSAL_LINES: usize = 8;
pub const G15_SAVE_LOAD_UX_SCHEMA: &str = "alife.g15.save_load_ux.v1";
pub const G15_SAVE_LOAD_UX_SCHEMA_VERSION: u16 = 1;
pub const G15_MAX_SAVE_SLOTS: usize = 4;
pub const G17_FEEDBACK_POLISH_SCHEMA: &str = "alife.g17.feedback_polish.v1";
pub const G17_FEEDBACK_POLISH_SCHEMA_VERSION: u16 = 1;
pub const G17_FEEDBACK_ASSET_MANIFEST_SCHEMA: &str = "alife.g17.feedback_asset_manifest.v1";
pub const G17_FEEDBACK_ASSET_MANIFEST_SCHEMA_VERSION: u16 = 1;
pub const G17_MAX_FEEDBACK_EVENTS: usize = 16;
pub const G17_MAX_POLISH_ASSET_BYTES: u64 = 8 * 1024;
pub const G18_POPULATION_PERFORMANCE_SCHEMA: &str = "alife.g18.population_performance.v1";
pub const G18_POPULATION_PERFORMANCE_SCHEMA_VERSION: u16 = 1;
pub const G18_MAX_TARGET_TIERS: usize = 6;
pub const G18_TARGET_FRAME_MS: f32 = 16.667;
pub const G19_LONG_RUN_BALANCE_SCHEMA: &str = "alife.g19.long_run_balance.v1";
pub const G19_LONG_RUN_BALANCE_SCHEMA_VERSION: u16 = 1;
pub const G19_FAST_BALANCE_CYCLES: u32 = 3;
pub const G19_EXTENDED_BALANCE_CYCLES: u32 = 16;
pub const G20_ONBOARDING_HELP_SCHEMA: &str = "alife.g20.onboarding_help.v1";
pub const G20_ONBOARDING_HELP_SCHEMA_VERSION: u16 = 1;
pub const G20_TUTORIAL_SCRIPT_SCHEMA: &str = "alife.g20.tutorial_script.v1";
pub const G20_TUTORIAL_SCRIPT_SCHEMA_VERSION: u16 = 1;
pub const G20_MAX_TUTORIAL_STEPS: usize = 8;
pub const G21_PLATFORM_PACKAGE_SCHEMA: &str = "alife.g21.platform_package.v1";
pub const G21_PLATFORM_PACKAGE_SCHEMA_VERSION: u16 = 1;
pub const G21_ASSET_BUNDLE_SCHEMA: &str = "alife.g21.asset_bundle.v1";
pub const G21_ASSET_BUNDLE_SCHEMA_VERSION: u16 = 1;
pub const G21_MAX_BUNDLE_ENTRIES: usize = 16;
pub const G22_PRODUCT_QA_SCHEMA: &str = "alife.g22.product_qa_hardening.v1";
pub const G22_PRODUCT_QA_SCHEMA_VERSION: u16 = 1;
pub const G22_MAX_QA_FINDINGS: usize = 32;
pub const G23_RELEASE_CANDIDATE_SCHEMA: &str = "alife.g23.playable_release_candidate.v1";
pub const G23_RELEASE_CANDIDATE_SCHEMA_VERSION: u16 = 1;
pub const G23_MAX_RELEASE_CANDIDATE_GATES: usize = 32;
pub const CA10_ENVIRONMENT_MANIFEST_SCHEMA: &str = "alife.ca10.environment_manifest.v1";
pub const CA10_ENVIRONMENT_MANIFEST_SCHEMA_VERSION: u16 = 1;
pub const CA10_MAX_ENVIRONMENT_SCENARIOS: usize = 8;
pub const CA11_PLAYER_SANDBOX_EDITOR_SCHEMA: &str = "alife.ca11.player_sandbox_editor.v1";
pub const CA11_PLAYER_SANDBOX_EDITOR_SCHEMA_VERSION: u16 = 1;
pub const CA12_APP_BUNDLE_MANIFEST_SCHEMA: &str = "alife.ca12.app_bundle_manifest.v1";
pub const CA12_APP_BUNDLE_MANIFEST_SCHEMA_VERSION: u16 = 1;
pub const CA12_PLACEHOLDER_ART_MANIFEST_SCHEMA: &str = "alife.ca12.placeholder_art_manifest.v1";
pub const CA12_PLACEHOLDER_ART_MANIFEST_SCHEMA_VERSION: u16 = 1;
pub const CA12_MAX_BUNDLE_FILE_BYTES: u64 = 64 * 1024;
/// Metadata packs referenced by the CA12 bundle may describe many bounded assets
/// while individual runtime fixtures remain subject to the smaller file cap.
pub const CA12_MAX_REFERENCED_MANIFEST_BYTES: u64 = 256 * 1024;
pub const CA12_MAX_BUNDLE_ENTRIES: usize = 32;
pub const CA13_DOUBLE_BUFFERED_SCHEDULER_SCHEMA: &str =
    "alife.ca13.double_buffered_graphical_scheduler.v1";
pub const CA13_DOUBLE_BUFFERED_SCHEDULER_SCHEMA_VERSION: u16 = 1;
pub const CA13_FIXED_SIM_TICK_HZ: u32 = 20;
pub const CA13_TARGET_RENDER_FRAME_HZ: u32 = 60;
pub const CA13_MAX_CATCH_UP_TICKS_PER_FRAME: u32 = 4;
pub const CA13_MAX_ACCUMULATOR_MICROS: u64 = 250_000;
pub const CA14_MOTOR_RING_PRESENTATION_SCHEMA: &str =
    "alife.ca14.motor_ring_arbitration_presentation.v1";
pub const CA14_MOTOR_RING_PRESENTATION_SCHEMA_VERSION: u16 = 1;
pub const CA14_MAX_MOTOR_RING_CHANNELS: usize = 6;
pub const CA15_HOMEOSTASIS_RUNTIME_SCHEMA: &str = "alife.ca15.endocrine_homeostasis_runtime.v1";
pub const CA15_HOMEOSTASIS_RUNTIME_SCHEMA_VERSION: u16 = 1;
pub const CA15_HOMEOSTASIS_REGISTER_COUNT: usize = 5;
pub const CA16_AFFORDANCE_LOOP_SCHEMA: &str = "alife.ca16.affordance_loop.v1";
pub const CA16_AFFORDANCE_LOOP_SCHEMA_VERSION: u16 = 1;
pub const CA16_EAT_REACH_DISTANCE: f32 = 1.25;
pub const CA17_HAZARD_RECOVERY_SCHEMA: &str = "alife.ca17.hazard_recovery_loop.v1";
pub const CA17_HAZARD_RECOVERY_SCHEMA_VERSION: u16 = 1;
pub const CA17_HAZARD_AVOIDANCE_DISTANCE: f32 = 1.75;
pub const CA18_GRAPHICAL_POPULATION_SCHEMA: &str = "alife.ca18.graphical_population.v1";
pub const CA18_GRAPHICAL_POPULATION_SCHEMA_VERSION: u16 = 1;
pub const CA18_MAX_GRAPHICAL_CREATURES: usize = 8;
pub const CA18_SOCIAL_CUE_DISTANCE: f32 = 2.75;
pub const CA19_GRAPHICAL_ECOLOGY_SCHEMA: &str = "alife.ca19.graphical_ecology.v1";
pub const CA19_GRAPHICAL_ECOLOGY_SCHEMA_VERSION: u16 = 1;
pub const CA19_MIN_TERRAIN_ZONES: usize = 2;
pub const CA19_MAX_TERRAIN_ZONE_VISUALS: usize = 8;

pub const CA20_GRAPHICAL_LIFECYCLE_SCHEMA: &str = "alife.ca20.graphical_lifecycle.v1";
pub const CA20_GRAPHICAL_LIFECYCLE_SCHEMA_VERSION: u16 = 1;
pub const CA20_MAX_LIFECYCLE_EVENT_ROWS: usize = 6;
pub const CA21_BEHAVIOR_TUNING_SCHEMA: &str = "alife.ca21.behavior_tuning_metrics.v1";
pub const CA21_BEHAVIOR_TUNING_SCHEMA_VERSION: u16 = 1;
pub const CA21_REQUIRED_DETECTOR_COUNT: usize = 5;
pub const CA21_SCENARIO_SWEEP_COUNT: usize = 5;
pub const CA22_ECOLOGICAL_SOAK_SCHEMA: &str = "alife.ca22.ecological_soak.v1";
pub const CA22_ECOLOGICAL_SOAK_SCHEMA_VERSION: u16 = 1;
pub const CA22_FAST_HEADLESS_TICKS: u32 = 1_000;
pub const CA22_MANUAL_HEADLESS_TICKS: u32 = 10_000;
pub const CA22_GRAPHICAL_BOUNDED_TICKS: u32 = 600;
pub const CA22_MAX_MANUAL_SOAK_TICKS: u32 = 10_000;
pub const CA23_GRAPHICAL_SCHOOL_SCHEMA: &str = "alife.ca23.graphical_school_mode.v1";
pub const CA23_GRAPHICAL_SCHOOL_SCHEMA_VERSION: u16 = 1;
pub const CA24_TEACHER_WORLD_CUES_SCHEMA: &str = "alife.ca24.teacher_world_cues.v1";
pub const CA24_TEACHER_WORLD_CUES_SCHEMA_VERSION: u16 = 1;
pub const CA25_CURRICULUM_AUTHORING_SCHEMA: &str = "alife.ca25.curriculum_authoring.v1";
pub const CA25_CURRICULUM_AUTHORING_SCHEMA_VERSION: u16 = 1;
pub const CA25_MAX_LESSONS: usize = 8;
pub const CA25_MAX_VERIFIER_CONDITIONS: usize = 8;
pub const CA26_REAL_SEMANTIC_PROVIDER_SCHEMA: &str = "alife.ca26.real_semantic_provider.v1";
pub const CA26_REAL_SEMANTIC_PROVIDER_SCHEMA_VERSION: u16 = 1;
pub const CA27_INTERNAL_SLM_PRIOR_SCHEMA: &str = "alife.ca27.internal_slm_prior.v1";
pub const CA27_INTERNAL_SLM_PRIOR_SCHEMA_VERSION: u16 = 1;
pub const CA32_REALTIME_WGSL_TELEMETRY_SCHEMA: &str = "alife.ca32.realtime_wgsl_telemetry.v1";
pub const CA32_REALTIME_WGSL_TELEMETRY_SCHEMA_VERSION: u16 = 1;
pub const CA33_BATCHED_GPU_RUNTIME_SCHEMA: &str = "alife.ca33.batched_gpu_runtime.v1";
pub const CA33_BATCHED_GPU_RUNTIME_SCHEMA_VERSION: u16 = 1;
pub const CA33_MAX_BATCH_CREATURES: usize = 30;
pub const CA33_MAX_BATCH_TICKS: u32 = 8;
pub const CA34_SAMPLED_GPU_RUNTIME_SCHEMA: &str = "alife.ca34.sampled_gpu_runtime.v1";
pub const CA34_SAMPLED_GPU_RUNTIME_SCHEMA_VERSION: u16 = 1;
pub const CA34_MAX_SAMPLED_TICKS: u32 = 16;
pub const CA36_SOAK_ISOLATION_SCHEMA: &str = "alife.ca36.multi_hour_soak_isolation.v1";
pub const CA36_SOAK_ISOLATION_SCHEMA_VERSION: u16 = 1;
pub const CA36_MIN_MANUAL_TICKS: u32 = 10_000;
pub const CA36_MIN_MULTI_HOUR_HOURS: u32 = 2;
pub const CA36_DEFAULT_REPORT_PATH: &str =
    "target/ca36_soak_isolation/CA36_MULTI_HOUR_SOAK_ISOLATION.md";
pub const CA37_WORLD_ART_STYLE_SCHEMA: &str = "alife.ca37.world_art_style.v1";
pub const CA37_WORLD_ART_STYLE_SCHEMA_VERSION: u16 = 1;
pub const CA37_MIN_PALETTE_MATERIALS: usize = 6;
pub const CA37_MIN_WORLD_DRESSING_PROPS: usize = 8;
pub const CA37_PROCEDURAL_VISUAL_MAP_WIDTH_TILES: usize = 4112;
pub const CA37_PROCEDURAL_VISUAL_MAP_HEIGHT_TILES: usize = 4112;
pub const CA37_MIN_PROCEDURAL_VISUAL_MAP_TILES: usize =
    CA37_PROCEDURAL_VISUAL_MAP_WIDTH_TILES * CA37_PROCEDURAL_VISUAL_MAP_HEIGHT_TILES;
pub const CA37_PROCEDURAL_VIEWPORT_WIDTH_TILES: usize = 17;
pub const CA37_PROCEDURAL_VIEWPORT_HEIGHT_TILES: usize = 11;
pub const CA38_CREATURE_ANIMATION_SCHEMA: &str = "alife.ca38.creature_animation_state_machine.v1";
pub const CA38_CREATURE_ANIMATION_SCHEMA_VERSION: u16 = 1;
pub const CA38_REQUIRED_ANIMATION_STATES: usize = 7;
pub const CA39_DRIVE_AUDIO_VFX_SCHEMA: &str = "alife.ca39.drive_coupled_audio_vfx.v1";
pub const CA39_DRIVE_AUDIO_VFX_SCHEMA_VERSION: u16 = 1;
pub const CA39_REQUIRED_DRIVE_CUE_COUNT: usize = 4;
pub const CA40_ONBOARDING_TUTORIAL_SCHEMA: &str = "alife.ca40.onboarding_tutorial.v1";
pub const CA40_ONBOARDING_TUTORIAL_SCHEMA_VERSION: u16 = 1;
pub const CA40_REQUIRED_CHECKLIST_ITEMS: usize = 5;
pub const CA42_RUNTIME_PREREQ_SCHEMA: &str = "alife.ca42.runtime_prereq_diagnostics.v1";
pub const CA42_RUNTIME_PREREQ_SCHEMA_VERSION: u16 = 1;
pub const CA43_TESTER_FEEDBACK_SCHEMA: &str = "alife.ca43.tester_feedback_capture.v1";
pub const CA43_TESTER_FEEDBACK_SCHEMA_VERSION: u16 = 1;
pub const CA43_MAX_SANITIZED_TEXT_BYTES: usize = 4096;
pub const CA44A_ALPHA_ART_MANIFEST_SCHEMA: &str = "alife.ca44a.alpha_art_manifest.v1";
pub const CA44A_ALPHA_ART_MANIFEST_SCHEMA_VERSION: u16 = 1;
pub const CA44A_MAX_ALPHA_ART_ASSET_BYTES: u64 = 64 * 1024;
pub const CA44A_MAX_ALPHA_ART_BACKDROP_BYTES: u64 = 768 * 1024;
pub const CA44A_MAX_ALPHA_ART_ENTRIES: usize = 40;
pub const CA44A_MIN_PRODUCTION_ART_DIMENSION: u32 = 96;
pub const CA44A_MAX_PRODUCTION_ART_DIMENSION: u32 = 640;
pub const CA44A_MAX_PRODUCTION_BACKDROP_DIMENSION: u32 = 1280;
pub const CA44A_REQUIRED_ALPHA_ART_ROLES: usize = 32;
pub const CA44A_STABILITY_SCHEMA: &str = "alife.ca44a.tick_stability.v1";
pub const CA44A_STABILITY_SCHEMA_VERSION: u16 = 1;
pub const CA44A_REQUIRED_STABILITY_TICKS: u32 = 600;
pub const TRUE_25D_ALPHA_ASSET_MANIFEST_SCHEMA: &str = "alife.ca44a.true_25d_asset_manifest.v1";
pub const TRUE_25D_ALPHA_ASSET_MANIFEST_SCHEMA_VERSION: u16 = 1;
pub const TRUE_25D_ALPHA_MIN_REQUIRED_ROLES: usize = 15;
pub const TRUE_25D_ALPHA_MAX_ASSET_BYTES: u64 = 32 * 1024;
pub const PRODUCTION_ASSET_PIPELINE_SCHEMA: &str =
    "alife.production.asset_pipeline.blender_2_5d.v1";
pub const PRODUCTION_ASSET_PIPELINE_SCHEMA_VERSION: u16 = 1;
pub const PRODUCTION_ASSET_PIPELINE_MIN_ROLES: usize = 14;
pub const PRODUCTION_ASSET_PIPELINE_MAX_ROLES: usize = 48;
pub const PRODUCTION_ASSET_PIPELINE_MAX_RENDER_SECONDS: u32 = 600;
pub const FVR07_PRODUCTION_ASSET_MANIFEST_SCHEMA: &str = "alife.fvr07.production_asset_manifest.v1";
pub const FVR07_PRODUCTION_ASSET_MANIFEST_SCHEMA_VERSION: u16 = 1;
pub const S01_GRAPHICAL_PLAYGROUND_SCHEMA: &str = "alife.s01.graphical_playground_shell.v1";
pub const S01_GRAPHICAL_PLAYGROUND_SCHEMA_VERSION: u16 = 1;
pub const S01_MAX_GRAPHICAL_SMOKE_SECONDS: u32 = 120;
pub const S02_RUNTIME_CONTROLS_SCHEMA: &str = "alife.s02.runtime_controls.v1";
pub const S02_RUNTIME_CONTROLS_SCHEMA_VERSION: u16 = 1;
pub const S02_MAX_RUN_TICKS_PER_UPDATE: u32 = 4;
pub const S02_MAX_SMOKE_TICKS: u32 = 16;
pub const S07_ADVANCED_GAMEPLAY_UX_SCHEMA: &str = "alife.s07.advanced_gameplay_ux.v1";
pub const S07_ADVANCED_GAMEPLAY_UX_SCHEMA_VERSION: u16 = 1;
pub const S08_GPU_GRAPHICS_PERFORMANCE_SCHEMA: &str = "alife.s08.gpu_graphics_performance.v1";
pub const S08_GPU_GRAPHICS_PERFORMANCE_SCHEMA_VERSION: u16 = 1;
pub const S08_TARGET_FPS: u16 = 60;
pub const S09_CONTENT_TUTORIAL_SCHEMA: &str = "alife.s09.content_tutorial_authoring.v1";
pub const S09_CONTENT_TUTORIAL_SCHEMA_VERSION: u16 = 1;
pub const S09_MAX_CONTENT_FILE_BYTES: u64 = 64 * 1024;
pub const FULL_GPU_NEURAL_RUNTIME_SCHEMA: &str = "alife.product.full_gpu_neural_runtime.v1";
pub const FULL_GPU_NEURAL_RUNTIME_SCHEMA_VERSION: u16 = 1;
pub const FVR01_PRODUCTION_FRONTEND_SCHEMA: &str = "alife.fvr01.production_voxel_frontend.v1";
pub const FVR01_PRODUCTION_FRONTEND_SCHEMA_VERSION: u16 = 1;
pub const FVR01_PROFILE_BUDGET_SCHEMA_VERSION: u16 = 1;
pub const FULL_GPU_NEURAL_RUNTIME_MAX_TICKS: u32 = 16;
pub const GPU_LONGRUN_SOAK_SCHEMA: &str = "alife.product.gpu_longrun_soak.v1";
pub const GPU_LONGRUN_SOAK_SCHEMA_VERSION: u16 = 1;
pub const GPU_LONGRUN_SOAK_DEFAULT_TICKS: u32 = 1000;
pub const GPU_LONGRUN_SOAK_DEFAULT_REPORT_EVERY: u32 = 100;
pub const GPU_LONGRUN_SOAK_MAX_TICKS_MANUAL: u32 = 10_000;
pub const GPU_SUSTAINED_LEARNING_SOAK_SCHEMA: &str = "alife.product.gpu_sustained_learning_soak.v1";
pub const GPU_SUSTAINED_LEARNING_SOAK_SCHEMA_VERSION: u16 = 1;
pub const GPU_SUSTAINED_LEARNING_SOAK_DEFAULT_TICKS: u32 = 1000;
pub const GPU_SUSTAINED_LEARNING_SOAK_DEFAULT_REPORT_EVERY: u32 = 100;
pub const GPU_SUSTAINED_LEARNING_SOAK_DEFAULT_EPISODE_TICKS: u32 = 32;
pub const GPU_SUSTAINED_LEARNING_SOAK_MAX_TICKS_MANUAL: u32 = 10_000;
