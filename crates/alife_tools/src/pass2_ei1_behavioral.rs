...
pub fn run_planned_manifest(
    ...
    for seed in planned_seeds(object)? {
        ...
        let manifest_trial =
            Era1TrialManifest::new(seed, family, subject, familiar, novel, 1, false, seed)
                .map_err(core_failure)?;
    ...
    write_raw(
        ...
        &RawArtifact {
            ...
            capability_metrics: capability_metrics(&scenario_id, &evidence),
            cognitive_work: cognitive_work(&evidence),
            evidence: &evidence,
        },
    )?;
    object.insert(
...
fn validate_identity(
    ...
    if identity.len() < 8
        ...
        || string(object, "configuration_hash")? != identity
        || string(command_object(object)?, "runner_id")? != RUNNER_ID
        || artifact_path(object, "raw_output_path").is_err()
        || artifact_path(object, "receipt_path").is_err()
    {
        return Err(Pass2Ei1BehavioralError::InvalidManifest(
