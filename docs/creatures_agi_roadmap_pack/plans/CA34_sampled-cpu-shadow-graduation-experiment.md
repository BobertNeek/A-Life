...
## Objective
Experiment with sampled CPU shadow while preserving fallback and no overclaim.
...
## Required work
- Warmup every-tick parity.
- Sample interval.
- Fallback on first failure.
- Manual mode only.
- Report claim as sampled, not full.
...
```text
CA34 receipt
Plan: Sampled CPU-shadow graduation experiment
Branch:
Files changed:
