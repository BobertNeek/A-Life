...
The cycle collects ordinary GPU decisions, streams Zstd-compressed binary replay records with a 4 GiB disk ceiling and 8 GiB process-memory check, predicts frozen values, applies PPO from sealed biological outcomes, exports a canonical N2048 asset, and verifies its admission into a fresh cohort. The sealed experience patch retains its existing JSON wire inside each binary record. The second command restores the exported actor, Adam state, and GPU value head, and increments the policy version. Both commands fail rather than overwrite an output directory. `cycle.json` and its actor/value checkpoints are integration evidence, not a full campaign checkpoint or a promoted founder.

Do not start an overnight campaign yet. Grounded demonstrations, complete behavioral development/acceptance panels, long-life replay/GAE across segments, verified parallel throughput, and a durable deadline/best-checkpoint coordinator remain unimplemented. The current development plan is [the training regimen](n2048-care-training-regimen-2026-09-21.md).
