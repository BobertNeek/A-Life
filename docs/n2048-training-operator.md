...
The cycle collects ordinary GPU decisions, streams compressed replay records with a 4 GiB disk ceiling and 8 GiB process-memory check, predicts frozen values, applies PPO from sealed biological outcomes, exports a canonical N2048 asset, and verifies its admission into a fresh cohort. The second command restores the exported actor, Adam state, and GPU value head, and increments the policy version. Both commands fail rather than overwrite an output directory. `cycle.json` and its actor/value checkpoints are integration evidence, not a full campaign checkpoint or a promoted founder.
