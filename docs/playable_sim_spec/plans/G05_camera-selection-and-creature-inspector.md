# G05 - Camera, selection, and creature inspector

Branch: `codex/G05-camera-selection-inspector`

Prerequisites: G04

Concurrency: Serial UI foundation.
...
Add player-facing navigation and inspection: camera controls, pause/step controls, selecting creatures/entities, and a diagnostic inspector that exposes drives, hormones, memory/topology summaries, current action, sleep state, and last sealed patch summary.
...
- selection/picking systems
- inspector UI panels
- read-only debug data adapters
...
- Inspector cannot mutate cognition unless a later editing plan owns it
- No active neural bulk readback
- No engine IDs in saves
- No overbuilt UX framework beyond current needs

## Implementation milestones

1. Implement camera pan/zoom/orbit or top-down controls suitable for the visible world.
2. Implement entity selection using Bevy-local entity mapping to stable IDs.
3. Add read-only creature inspector panel.
4. Add pause/step/run speed controls.
5. Add last action/outcome/patch summary display.
6. Add troubleshooting messages for CPU/GPU fallback and missing optional providers.

## Required tests and evidence

- Selection maps Bevy entity to stable ID.
- Inspector displays expected fields for a tiny run.
- Pause/step controls deterministic.
- Core boundary clean.

## Acceptance criteria

- A player can select one creature and understand what it is doing.
- Inspector is read-only for cognition.
- Selection uses stable IDs for model data and Bevy IDs only locally.
...
- The plan implemented only `G05` scope.
...
```text
Completion receipt
Plan: G05 - Camera, selection, and creature inspector
Branch: codex/G05-camera-selection-inspector
Files changed:
Public APIs changed:
Tests added/changed:
