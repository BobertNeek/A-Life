# G12 - GPU product hardening and runtime telemetry

Branch: `codex/G12-gpu-product-hardening`

Prerequisites: G03, P29 complete

Concurrency: Serial high-risk GPU/product gate.
...
Convert the optional GPU runtime from diagnostic/fallback plumbing into a product-safe option inside the playable app with honest telemetry, CPU fallback, no-readback enforcement, and measured performance where hardware is available.
...
- game app GPU runtime selection
- GPU telemetry overlay
- performance report bridge
- manual hardware run docs
- fallback UX
...
- Making GPU required
- Claiming GPU FPS without hardware evidence
- Synchronous active neural readback
- Changing CPU oracle to match GPU

## Implementation milestones

1. Expose GPU backend selection in config/UI with CPU fallback.
2. Show backend selected/fallback reason in UI.
3. Integrate P29 no-readback guard into app active path.
4. Add telemetry overlay for tick time, backend, skipped tiles, fallback status.
5. Add manual hardware performance command and report template.
6. Run optional local hardware tests if available and record honestly.

## Required tests and evidence

- CPU fallback default.
- Invalid GPU config falls back.
- No active readback API callable from app.
- Telemetry report created.
- Manual GPU commands documented.

## Acceptance criteria

- Game runs on CPU fallback without GPU.
- GPU option cannot introduce active readback.
- Telemetry distinguishes measured GPU from CPU fallback.
...
- The plan implemented only `G12` scope.
...
```text
Completion receipt
Plan: G12 - GPU product hardening and runtime telemetry
Branch: codex/G12-gpu-product-hardening
Files changed:
Public APIs changed:
Tests added/changed:
