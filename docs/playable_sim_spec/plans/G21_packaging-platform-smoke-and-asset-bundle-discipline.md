# G21 - Packaging, platform smoke, and asset bundle discipline

Branch: `codex/G21-packaging-platform-smoke`

Prerequisites: G20, G15

Concurrency: Serial platform gate.
...
Prepare local build/package discipline without claiming final release: Windows-first run scripts, asset bundle layout, config defaults, and platform smoke commands for headless and graphical paths.
...
- Publishing release without user approval
- Code signing/store integration unless requested
- Committing large build outputs
- Breaking cargo workspace defaults

## Implementation milestones

1. Define local build output directory under target/artifacts or dist ignored.
2. Add run scripts for headless and graphical app using correct repo paths.
3. Validate asset bundle references.
4. Add platform smoke checklist for Windows and generic non-Windows.
5. Ensure no generated build artifacts are tracked.

## Required tests and evidence

- Run script dry-run or help smoke.
- Asset bundle manifest validates.
- No dist/target artifacts tracked.
- Default cargo validation still passes.

## Acceptance criteria

- Developer can build/run locally from a documented command set.
- Package smoke does not require unsupported hardware by default.
...
- The plan implemented only `G21` scope.
...
```text
Completion receipt
Plan: G21 - Packaging, platform smoke, and asset bundle discipline
Branch: codex/G21-packaging-platform-smoke
Files changed:
Public APIs changed:
Tests added/changed:
