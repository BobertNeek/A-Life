...
- The supervisor alone updates the shared board and integration checkout. Workers use isolated app worktrees and commit only their issue files when their normal sandbox permits it. If Git is blocked, stop with the exact worktree, intended operation and allowlisted edits in a final handoff. Bob owns routine approvals and executes authorized Git work through his permitted tools, then resumes the worker. Do not request elevated Git approval from the user or change sandbox settings. Do not push, merge, update other tasks, or write the shared board. A plain final handoff is sufficient; no separate cross-task message tool is needed.
