...
function Assert-ManifestIdentity {
    ...
    foreach ($field in @('source_commit', 'configuration_hash', 'scenario_corpus_version', 'brain_class', 'population', 'stage')) {
...
if ($Mode -eq 'execute') {
    ...
    foreach ($row in $manifestRows) {
        ...
        $output = (& $RunnerCommand[0] @($RunnerCommand[1..($RunnerCommand.Count - 1)] + @('--planned-manifest', $plannedPath, '--manifest-identity', $row.cache_key)) 2>&1 | Out-String)
